...
- [x] Go 编译通过 (`go build ./...`)
- [x] Wails 构建成功 (`wails build` → `build/bin/yx-daq.exe`)
...
- [x] Wails NSIS集成: `wails build --target windows/amd64 --nsis`
...
```
yx-daq/build/bin/yx-daq.exe              (Windows 可执行文件, 1440×900 窗口)

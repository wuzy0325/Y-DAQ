  ...
  <div class="main-layout">
    <header v-if="!isPopoutWindow" class="topbar">
      ...
      <nav class="topbar-nav">
        ...
        <span class="nav-item nav-dropdown" @click="openMotionWindow">
          <span class="nav-icon">🎯</span>
          <span class="nav-label">运动控制</span>
        </span>
        ...
        <span class="nav-item nav-dropdown" @click="openFiveHoleWindow">
          <span class="nav-icon">🔬</span>
          <span class="nav-label">五孔插值移位测试</span>
        </span>
      </nav>
...
import { ThreeHoleService, MotionService, FiveHoleService } from '@/wails-compat/services'
import { ArrowDown } from '@element-plus/icons-vue'

const route = useRoute()
const deviceStore = useDeviceStore()

// 独立弹窗（探针测试 / 运动控制窗口）隐藏顶部导航栏
const isPopoutWindow = computed(() => !!route.query.probe || !!route.query.window)
...
async function openMotionWindow() {
  try {
    await MotionService.OpenMotionWindow()
  } catch (e) {
    console.error('openMotionWindow failed:', e)
  }
}

// 打开五孔插值移位测试独立窗口（单实例，管理 1-3 探针）
async function openFiveHoleWindow() {
  try {
    await FiveHoleService.OpenTestWindow()
  } catch (e) {
    console.error('openFiveHoleWindow failed:', e)
  }
}

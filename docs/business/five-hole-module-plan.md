...
| .cal 格式未定 | 占位 `FiveHoleCalibEntry`，解析器先按参考文档六列实现，待算法确认后调整 |
...
- **内容**：
  ...
  - `LoadCalibFiles(filePaths []string) error`（解析 .cal，按占位六列格式，校验所有文件 α/β 序列一致）
  ...
  - `parseCalFile(path string) (*FiveHoleCalibData, error)`（占位解析器）
...
- **内容**：
  - `internal/app/service_five_hole.go`：
    ...
    - `SelectFiveHoleCalibFiles() []string`（文件选择对话框，过滤 *.cal）
...
- **内容**：
  - `frontend/src/views/FiveHoleTestView.vue`：
    ...
    - 设置弹窗 `el-dialog`：
      ...
      - Tab 2 探针配置：1-3 探针启用开关 + 每探针 P1-P5 通道映射表（含设备选择）+ α/β 轴（位移机构+轴号）+ .cal 载入
...
- 具体五孔插值算法实现（T2 占位）
- `.cal` 校准文件实际格式（T2 占位解析器）
- 多/单 cal 索引方式（T2 占位，待算法确认）

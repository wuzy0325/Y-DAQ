...
func (c *Core) Startup(app *application.App) {
	...
	go c.AcquisitionHub.StartPublishing(c.publishCtx)
...
func (c *Core) Shutdown() {
	// 取消 publishCtx，通知所有监听协程退出
	// （StartPublishing、broadcastMotionStatus、broadcastDeviceStatus 都监听此 ctx）
	...
	// profile 已由各 manager 的 AddProfile/UpdateProfile/RemoveProfile 等方法
	// 内部调用 saveProfiles() 实时持久化，无需在 Shutdown 重复保存。
...
// getChannelValues 从最新数据快照中读取指定通道值。
// deviceID 为空时跨所有设备查找首个匹配，非空时仅在该设备快照中查找。
func (c *Core) getChannelValues(deviceID string, channels []int) map[int]float64 {
	result := make(map[int]float64, len(channels))
	snapshots := c.AcquisitionHub.GetSnapshot()
	for _, wantCh := range channels {
		for _, snap := range snapshots {
			if deviceID != "" && snap.DeviceID != deviceID {
				continue
			}
			for i, idx := range snap.ChannelIndices {
				if idx == wantCh && i < len(snap.Channels) {
			...
			if _, found := result[wantCh]; found {
				break
			}
		}
	}
...
func (c *Core) createThreeHoleService(probeID string) *three_hole.ThreeHoleTraversalService {
	...
	svc.SetBatchGetter(func(channels []types.ThreeHoleProbeChannelConfig) (map[int]float64, error) {
		...
		chNums := make([]int, 0, len(channels))
		for _, ch := range channels {
			if ch.Enabled {
				chNums = append(chNums, ch.Channel)
			}
		}
		return c.getChannelValues(config.DeviceID, chNums), nil
	})
...
func (c *Core) broadcastStatus(interval time.Duration, get func() any, eventName string) {
	...
	for {
		select {
		case <-c.publishCtx.Done():
			return
		case <-ticker.C:
			data := get()
			curJSON, _ := json.Marshal(data)
			if string(curJSON) == lastJSON {
				continue
			}
			lastJSON = string(curJSON)
			c.App.Event.Emit(eventName, data)
		}
	}
}

// broadcastMotionStatus 定时推送运动控制器状态。
// 仅在状态变化时推送：运动控制器静止时轮询值不变，避免 10Hz
// 重复推送相同状态造成前端 axisUIStates 的无谓响应式刷新。
func (c *Core) broadcastMotionStatus() {
	c.broadcastStatus(
		time.Duration(types.MotionPollIntervalMs)*time.Millisecond,
		func() any { return c.MotionManager.GetCachedStatusAll() },
		"motion:status-updated",
	)
}

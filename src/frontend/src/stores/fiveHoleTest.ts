...
export const useFiveHoleTestStore = defineStore('fiveHoleTest', () => {
  ...
  async function selectCalibFiles(probeId: string) {
    try {
      const files = await FiveHoleService.SelectFiveHoleCalibFiles() as string[]
      if (files && files.length > 0) {
        await FiveHoleService.LoadFiveHoleCalibFiles(probeId, files)
        const infos = await FiveHoleService.GetFiveHoleCalibInfo(probeId) as FiveHoleCalibFileInfo[]
  ...
  async function ensureDevicesAcquiring() {
    ...
    // 与原实现行为一致：每个失败设备的错误依次写入 lastError（最后一个生效）
    for (const e of errs) lastError.value = e
  }

  // 重置运行时状态：taskStatus/realtime/progress
  // 用于启动前清空与失败回滚（completeProbeDataPoints 仅启动前清理，不在回滚中重复）
  ...
  async function pauseTest() {
    try {
      await FiveHoleService.PauseFiveHoleTraversal()
      await fetchStatus()
    } catch (e) {
  ...
  async function resumeTest() {
    try {
      await FiveHoleService.ResumeFiveHoleTraversal()
      await fetchStatus()
    } catch (e) {
  ...
  async function stopTest() {
    try {
      await FiveHoleService.StopFiveHoleTraversal()
      let retries = 0
      const maxRetries = 10
  ...
  async function startRealtimeMonitor() {
    try {
      await FiveHoleService.StartFiveHoleRealtimeMonitor(config.value as any)
    } catch (e) {
  ...
  async function stopRealtimeMonitor() {
    try {
      await FiveHoleService.StopFiveHoleRealtimeMonitor()
    } catch (e) {
  ...
  async function selectAndStartRealtimeRecording() {
    try {
      return await FiveHoleService.SelectAndStartFiveHoleRealtimeRecording()
    } catch (e) {
  ...
  async function stopRealtimeRecording() {
    try {
      await FiveHoleService.StopFiveHoleRealtimeRecording()
    } catch (e) {
  ...
  async function fetchStatus() {
    try {
      taskStatus.value = await FiveHoleService.GetFiveHoleTraversalStatus() as FiveHoleTraversalTaskStatus
    } catch (e) {
  ...
  const eventListener = createWailsEventListener('five-hole', [
    { channel: 'five-hole:progress', handler: (event: any) => { progress.value = event.data as FiveHoleTraversalProgressEvent } },
    { channel: 'five-hole:realtime', handler: (event: any) => { realtime.value = event.data as FiveHoleTraversalRealtimeEvent } },
    {
      channel: 'five-hole:complete',
  ...
  function exportProbeCSV(probeId: string) {
    ...
    const rows = probeData.map(p => [
      ...
      p.interpResult.casProbe.toFixed(4), p.interpResult.satProbe.toFixed(4),
      p.interpResult.dynamicPressure.toFixed(6), p.interpResult.density.toFixed(6),
      p.interpResult.vxProbe.toFixed(4), p.interpResult.vyProbe.toFixed(4),
      p.interpResult.vzProbe.toFixed(4),
      p.sampleCount.toString(), p.timestamp.toString(),
    ])

    downloadCSV(`five-hole-${probeId}`, headers, rows)
  }
  ...
  function saveConfig() {
    try {
      ...
      FiveHoleService.SaveFiveHoleConfig(config.value as any).catch((e: unknown) => {
  ...
  async function loadConfig() {
    ...
    for (const probe of config.value.probes) {
      ...
      try {
        await FiveHoleService.LoadFiveHoleCalibFiles(probe.probeId, filePaths)
        const infos = await FiveHoleService.GetFiveHoleCalibInfo(probe.probeId) as FiveHoleCalibFileInfo[]

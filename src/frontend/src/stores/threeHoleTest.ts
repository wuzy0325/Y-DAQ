...
import { ThreeHoleService, ConfigService } from '@bindings/yx-daq/internal/app'
import {
...
export const useThreeHoleTestStore = defineStore('threeHoleTest', () => {
  ...
  async function selectCalibFiles() {
    try {
      ...
      if (files && files.length > 0) {
        ...
        await ThreeHoleService.LoadThreeHoleCalibFiles(probeID.value, files)
        const infos = await ThreeHoleService.GetThreeHoleCalibInfo(probeID.value) as { cMa: number }[]
  ...
  async function ensureDeviceAcquiring() {
    ...
    if (!deviceId) return
    const errs = await ensureDevicesAcquiring([deviceId])
    if (errs.length > 0) lastError.value = errs[0]
  }
  ...
  async function startTest() {
    ...
    try {
      await ThreeHoleService.StartThreeHoleTraversal(probeID.value, config.value as any)
  ...
  async function pauseTest() {
    try {
      await ThreeHoleService.PauseThreeHoleTraversal(probeID.value)
      await fetchStatus()
    } catch (e) {
  ...
  async function resumeTest() {
    try {
      await ThreeHoleService.ResumeThreeHoleTraversal(probeID.value)
      await fetchStatus()
    } catch (e) {
  ...
  async function stopTest() {
    try {
      await ThreeHoleService.StopThreeHoleTraversal(probeID.value)
      // 轮询状态直到确认停止，最多等待2秒
      let retries = 0
      const maxRetries = 10
  ...
  async function startRealtimeMonitor() {
    try {
      await ThreeHoleService.StartThreeHoleRealtimeMonitor(probeID.value, config.value as any)
    } catch (e) {
  ...
  async function stopRealtimeMonitor() {
    try {
      await ThreeHoleService.StopThreeHoleRealtimeMonitor(probeID.value)
    } catch (e) {
  ...
  async function fetchStatus() {
    try {
      taskStatus.value = await ThreeHoleService.GetThreeHoleTraversalStatus(probeID.value) as ThreeHoleTraversalTaskStatus
    } catch (e) {
  ...
  function startListening() {
    ...
    try {
      Events.On(`three-hole:${pid}:progress`, (event: any) => {
        progress.value = event.data as ThreeHoleTraversalProgressEvent
      })
      Events.On(`three-hole:${pid}:realtime`, (event: any) => {
        realtime.value = event.data as ThreeHoleTraversalRealtimeEvent
      })
      Events.On(`three-hole:${pid}:complete`, async (_event: any) => {
        progress.value = null
        await fetchStatus()
      })
      Events.On(`three-hole:${pid}:error`, (event: any) => {
        const data = event.data as ThreeHoleTraversalErrorEvent
  ...
  function stopListeningFor(pid: string) {
    try {
      Events.Off(`three-hole:${pid}:progress`)
      Events.Off(`three-hole:${pid}:realtime`)
      Events.Off(`three-hole:${pid}:complete`)
      Events.Off(`three-hole:${pid}:error`)
    } catch (e) {
  ...
  function exportCSV() {
    ...
    downloadCSV(`three-hole-traversal-${probeID.value}`, headers, rows)
  }
  ...
  function saveConfig() {
    try {
      ...
      const p = probeID.value
        ; (p === 'probe2' ? ConfigService.SaveThreeHoleProbe2Config(cfg) : ConfigService.SaveThreeHoleProbe1Config(cfg))
          .catch((e: unknown) => {
  ...
  async function loadConfig() {
    try {
      // 优先从后端加载（按 probeID 路由到对应的 ConfigService 方法）
      try {
        const p = probeID.value
        const loaded = await (p === 'probe2' ? ConfigService.LoadThreeHoleProbe2Config() : ConfigService.LoadThreeHoleProbe1Config()) as any
    ...
    if (savedCalibFiles && savedCalibFiles.length > 0) {
      ...
      if (filePaths.length > 0) {
        try {
          await ThreeHoleService.LoadThreeHoleCalibFiles(probeID.value, filePaths)
          const infos = await ThreeHoleService.GetThreeHoleCalibInfo(probeID.value) as { cMa: number }[]

...
import { ConfigService, ThreeHoleService } from '@bindings/yx-daq/internal/app'
...
const { isRecording, handleStartRecording, handleStopRecording } = useTestRealtimeRecording({
  startRecording: () => ThreeHoleService.SelectAndStartThreeHoleRealtimeRecording(probeParam),
  stopRecording: () => ThreeHoleService.StopThreeHoleRealtimeRecording(probeParam),
})
...
const { pointCanvasRef, previewPoints, drawPointCanvas } = usePointPreviewCanvas({
  ...
  padding: { left: 48, right: 16, top: 16, bottom: 40 },
  pointRadiusFactor: 140,
})
...
// 测试状态变化时的处理（启动/停止均清除历史数据并刷新图表）
watch(() => store.isRunning, () => {
  ptHistory.value = []
  psHistory.value = []
  maHistory.value = []
  alphaHistory.value = []

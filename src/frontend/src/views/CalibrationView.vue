...
import { DataService } from '@bindings/yx-daq/internal/app'
import { downloadCSV } from '../utils/csv'

const calibStore = useCalibrationStore()

onMounted(() => {
  // 注册事件监听，否则 realtime/progress 事件不会更新 UI
  calibStore.startListening()
  calibStore.fetchStatus()
})

onUnmounted(() => {
  calibStore.stopListening()
})
...
function exportCSV() {
  ...
  downloadCSV('calibration', headers, rows)
}
